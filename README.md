# Ready, Steady, Connect - A Kafka Connect ETL with custom-transformation

*'Ready, Steady, Connect - A Kafka Connect ETL'* is an example project to play around
...
The following components are part of the Kafka Connect ETL with custom-transformation project:
...
The [`FlattenArray`](src/main/java/ch/yax/connect/quickstart/transforms) transforms all nested and array type data. This transform can
be used to add complex data to the Kafka message.
...
After the transform of the above message.

```json
{
    "organizaton": "Hishab Technology",
    "services": {IVR,VoiceBridge,AUDIT},
    "blocklist": {01779xxxxx,01778xxxxx,01716xxxxx},
    "location": "Dhaka",
	"nested_address": "Gulshan",
	"nested_road": "Bashtola_Road"
}
```
...
```bash
quickstart-connect | [2021-02-10 22:09:22,907] INFO Topic random-data-avro, created connect data 'SchemaAndValue{schema=Schema{com.kafka.connect.etl.source.RandomData:STRUCT}, value=Struct{value=7476346340551272290,count=632,message=Task Id: 1,timestamp=Wed Feb 10 22:09:22 GMT 2021}}' (com.kafka.connect.etl.converter.AvroDebugConverter)
```

It will also log the data structure which was received.

```bash
quickstart-connect | [2021-02-10 22:09:28,377] INFO Topic random-data-avro, got connect data 'Struct{value=6085501730739912467,count=633,message=Task Id: 1,timestamp=Wed Feb 10 22:09:28 GMT 2021}' and schema 'Schema{com.kafka.connect.etl.source.RandomData:STRUCT}' (com.kafka.connect.etl.converter.AvroDebugConverter)
```
...
```properties
value.converter=com.kafka.connect.etl.converter.JsonDebugConverter
```
...
```bash
[2021-02-10 23:09:41,815] INFO metricChange: name: MetricName [name=record-send-total, group=producer-topic-metrics, description=The total number of records sent for a topic., tags={client-id=producer-1, topic=docker-connect-offsets}], value: 0.0 (com.kafka.connect.etl.metrics.LogMetricsReporter)
```
